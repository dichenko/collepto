...
// POST create new item
  router.post('/', async (c) => {
    try {
      ...
      await db.logAdminAction('create', 'item', itemId, validation.data);
...
router.put('/:id', async (c) => {
  try {
    ...
    if (updated) {
      await db.logAdminAction('update', 'item', id, validation.data);
    }
...
router.delete('/:id', async (c) => {
  try {
    ...
    await db.logAdminAction('delete', 'item', id);
...
// GET search items (with FTS5 if q provided)
router.get('/search', async (c) => {
  try {
    ...
    // If FTS query present, start from FTS
    let sql = query
    ...
    // Tags filter - JSON array contains ANY of provided tags
    if (tags.length > 0) {
