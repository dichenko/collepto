...
router.post('/', async (c) => {
  try {
    ...
    await db.logAdminAction('create', 'blog_post', postId, validation.data);
...
router.put('/:id', async (c) => {
  try {
    ...
    if (updated) {
      await db.logAdminAction('update', 'blog_post', id, validation.data);
    }
...
router.delete('/:id', async (c) => {
  try {
    ...
    await db.logAdminAction('delete', 'blog_post', id);
...
router.post('/:id/publish', async (c) => {
  try {
    ...
    await db.logAdminAction('publish', 'blog_post', id, { published });
...
router.get('/search', async (c) => {
  try {
    ...
    let sql = query
      ? `SELECT blog_posts.* FROM blog_posts_fts \
           JOIN blog_posts ON blog_posts_fts.rowid = blog_posts.rowid \
